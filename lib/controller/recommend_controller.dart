  ...
  Future<void> loadRecommendCourses() async {
    ...
    for (int i = 1; i <= 8; i++) {
      var temp = await db
          ...
          .doc('semester$i')
          .get();
      for (var course in (temp.data()!['must'] as List)) {
        must.add(course);
      }
      for (var course in (temp.data()!['select'] as List)) {
        select.add(course);
      }

      // 이미 수강한 수업 제외
      for (var course in UserController.to.userCourses) {
        if (must.contains(course.name)) {
          must.remove(course.name);
        }
        if (select.contains(course.name)) {
          select.remove(course.name);
        }
      }
      ...
      if (UserController.to.englishGrade > 0) {
        must.remove("English Foundation");
      }
      ...
      for (var course in must.toList()) {
        if (course.contains("사회봉사") && flag1 == 1) {
          must.remove(course);
        }
        if (course.contains("공동체 리더십 훈련") && flag2 == 1) {
          must.remove(course);
        }
        if (course.contains("채플") && flag3 == 1) {
          must.remove(course);
        }
        if (course.contains("Calculus") && flag4 == 1) {
          must.remove(course);
        }
        ...
        if (course.contains("사회봉사") && flag1 == 0) {
          flag1 = 1;
        }
        if (course.contains("공동체 리더십 훈련") && flag2 == 0) {
          flag2 = 1;
        }
        if (course.contains("채플") && flag3 == 0) {
          flag3 = 1;
        }
      ...
      int count = 0;
      while (must.isNotEmpty && count < 10 && i > UserController.to.semester) {
      ...
      while (
          count < 10 && select.isNotEmpty && i > UserController.to.semester) {
